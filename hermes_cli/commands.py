"""Slash command registry for the Ettok CLI and gateway.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Ettok config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Ettok works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Ettok' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]", subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Ettok is working", "Configuration",
    ...
    CommandDef("update", "Update Ettok AI to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Ettok AI version", "Info", aliases=("v",),
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``ettok skills list`` and the agent's ``/skill-name`` dispatch but
