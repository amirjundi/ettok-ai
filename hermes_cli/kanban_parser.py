"""Argparse tree for ``ettok kanban …`` (``build_parser``).
...
_BOARD_SPECS = [
    ...
    _cmd("import", [
        ...
        "Import a .tar.gz produced by `ettok kanban boards export`. The board always lands as a "
...
# Top-level ``ettok kanban <action>`` records, in ``--help`` order.
_SPECS = [
    ...
    _cmd("create", [
        ...
        _arg("--project",
             help="Link to a project (id or slug). Anchors the task's "
                  ...
                  "deterministic branch. See `ettok project list`."),
        _TENANT,
        _PRIORITY,
    ...
    ], help="DEPRECATED — dispatcher now runs in the gateway. Use `ettok gateway start`."),
    _cmd("watch", [
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description="Durable SQLite-backed task board shared across Ettok profiles. "
    ...
    kanban_parser.add_argument("--board", default=None, metavar="<slug>",
                               help="Board slug to operate on. Defaults to the current board (set "
                                    "via `ettok kanban boards switch <slug>` or the "
                                    "HERMES_KANBAN_BOARD env var). Use `ettok kanban boards "
