...
# platforms; config values and instruction/context .md files may not map 1:1 to Ettok.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Ettok to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Ettok at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Ettok at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Ettok at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Ettok at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Ettok equivalents",
...
_MIGRATION_WARNING_NOTES = (
    "  Note: OpenClaw config values may have different semantics in Ettok.",
    "  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Ettok's yolo mode.",
...
def _print_migration_preview(report: dict):
    ...
    groups = (
        ...
        ("conflict", "  Would overwrite (conflicts with existing Ettok config):", Colors.YELLOW,
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    _info(f"Found OpenClaw data at {openclaw_dir}",
          "Ettok can preview what would be imported before making any changes.", None)
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info("Skipping migration. You can run it later with: ettok claw migrate --dry-run")
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        _info("Migration cancelled. You can run it later with: ettok claw migrate",
              "Use --dry-run to preview again, or --preset minimal for a lighter import.")
        return False

    # overwrite=False so existing Ettok configs are preserved. The user saw the preview;
    ...
    for key, printer, text in (
        ...
        ("conflict", print_info,
         "Skipped {n} item(s) that already exist in Ettok (use ettok claw migrate --overwrite to force)."),
