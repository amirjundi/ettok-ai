...
def _discovered_catalog_stale(entry: dict, model_ids: list[str]) -> bool:
    ...
    A mapping Ettok itself discovered (entry flag or legacy in-mapping sentinel) is ours to
...
def _live_or_curated_ids(slug: str, curated: dict, *fallback_keys: str, merge_models_dev: bool = True) -> list:
    """``cached_provider_model_ids`` (the SAME disk-cached list ``ettok model`` builds), falling
...
def _display_prefix(name: str) -> str:
    """Text before the per-model separator Ettok's own writer uses ("—" / " - ")."""
...
class _PickerBuild:
    ...
    models.dev, 2 Ettok-only overlays, 2b canonical providers missed by 1/2, 3 ``providers:``
...
def _lap_overlay_rows(b: _PickerBuild, data: dict) -> None:
    """Section 2: Ettok-only providers (nous, openai-codex, copilot, opencode-go, ...)."""
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from hermes_cli.providers import HERMES_OVERLAYS
