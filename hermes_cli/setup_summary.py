...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'ettok setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'ettok setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'ettok tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("ettok setup", "          Re-run the full wizard"), ("ettok setup model", "    Change model/provider"),
    ("ettok setup terminal", " Change terminal backend"), ("ettok setup gateway", "  Configure messaging"),
    ("ettok setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("ettok config", "         View current settings"), ("ettok config edit", "    Open config in your editor"),
    ("ettok config set <key> <value>", ""))
_READY_ROWS = (
    ("hermes", "              Start chatting"), ("ettok gateway", "      Start messaging gateway"),
    ("ettok doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'ettok setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `ettok tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'ettok setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via ettok auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — Ettok cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    ettok model            (pick any provider/model)",
              "    ettok setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'ettok setup tools' to configure them,")
