"""Streamlined setup flows: the Nous Portal one-shot (`ettok portal`), first-time quick setup,
...
def _blank_slate_done(config: dict, hermes_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    ...
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         ettok skills opt-in --sync",
          "  Add MCP servers:     ettok mcp add", *extra, "  Tune agent settings: ettok setup agent", None)
...
def _run_nous_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    ...
    single source of truth shared with ``ettok model``. False when cancelled or failed (the
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Nous Portal setup (``ettok setup --portal`` / ``ettok portal``)."""
    from hermes_cli.setup import _info, _print_banner, print_error, print_info, print_success
    _print_banner("│     ℹ Ettok Setup — Nous Portal (one-shot)             │")
    ...
    def _on_error(exc: Exception) -> None:
        ...
        print_info("  You can retry later with `ettok portal`.")

    if not _run_nous_flow(config, context="`ettok portal`", cancel_exc=(KeyboardInterrupt, EOFError, SystemExit),
                          cancel_lines=(None, "  Setup cancelled.", "  You can retry later with `ettok portal`."),
    ...
    _info("  Run `ettok portal info` to inspect routing.", "  Run `hermes` to start chatting.")
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    def _on_error(exc: Exception) -> None:
        ...
        print_info("You can try again later with: ettok model")
    ...
    gateway_choice = prompt_choice("Connect a messaging platform? (Telegram, Discord, etc.)", [
        "Set up messaging now (recommended)", "Skip — set up later with 'ettok setup gateway'",
    ], 0)
    if gateway_choice == 0:
        ...
        # platforms come alive as soon as tokens are added later (e.g. via `ettok import`).
        from hermes_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    print()
    print_success("Setup complete! You're ready to go.")
    _info(None, "  Configure all settings:    ettok setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  ettok setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  The grant is permanent — it survives every Ettok update.")
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``ettok setup agent``."""
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "can help you drive and configure Ettok itself.", None)
    ...
    if path != 0:
        ...
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `ettok update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, hermes_home, "  Enable tools:        ettok tools", "  Enable plugins:      ettok plugins",
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `hermes-agent`",
              ...
              "`ettok update` runs from re-injecting them. Opt back in any",
              "time with `ettok skills opt-in --sync`.")
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `ettok tools`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `ettok plugins list` / `ettok plugins install`.",
         "No plugins enabled. Add later with `ettok plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `ettok mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `ettok mcp add`."),
    ):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, hermes_home, "  Enable more tools:   ettok tools")
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'ettok setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Ettok to messaging apps to chat from anywhere.",
              "You can configure these later with 'ettok setup gateway'.")
