...
Handles: ettok gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# its full budget and `ettok update` can deadlock. Two witnesses classify the loop BEFORE any
...
# fail", and `ettok update` can deadlock behind it. The loop publishes a liveness signal precisely for this
...
def _get_ancestor_pids() -> set[int]:
    ...
    process or any of its parents. This prevents ``ettok gateway status`` from falsely counting the
...
def _scan_gateway_pids(
    ...
    # Exclude the entire ancestor chain so the CLI process that invoked this scan (e.g. ``ettok gateway
...
def _windows_process_listing() -> str | None:
    ...
    # failure or timeout (result is None) trips the fallback. ``ettok update`` hung exactly there on
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    """Find running gateway PIDs for the current profile, or every profile with ``all_profiles`` (``ettok update``)."""
...
def find_profile_gateway_processes(exclude_pids: set | None = None, *, strict: bool = False) -> list[ProfileGatewayProcess]:
    """Return running gateway PIDs mapped to Ettok profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def find_windows_gateway_services(
    *, psutil_module=None, profile_processes: list[ProfileGatewayProcess] | None = None
    ...
    command lines, so identity = Ettok's own PID file + a parent chain ending at a running SCM service
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``ettok update``: ``--external-supervisor`` gateways
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {sudo}ettok gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {sudo}ettok gateway restart{scope_flag}")
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: ettok gateway stop")
    else:
        print("⚠ Gateway process is running for this profile, but the service is not active")
        print(pids_line)
        print("  This is usually a manual foreground/tmux/nohup run, so `ettok gateway`")
        print("  can refuse to start another copy until this process stops.")


def _print_other_profiles_gateway_status() -> None:
    """Print other profiles' running gateways at the bottom of ``ettok gateway status``."""
    try:
...
SERVICE_DESCRIPTION = "Ettok AI Gateway - Messaging Platform Integration"
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    ...
    Detects unit files installed by older Ettok versions that used a different service name (e.g. When both
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Ettok gateway unit files exist."""
...
def print_legacy_unit_warning() -> None:
    ...
    if not legacy:
        return
    print_warning("Legacy Ettok gateway unit(s) detected from an older install:")
    ...
    print_info("    ettok gateway migrate-legacy")
...
def remove_legacy_hermes_units(interactive: bool = True, dry_run: bool = False) -> tuple[int, list[Path]]:
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Ettok gateway units found.")
        return 0, []

    print()
    print("Legacy Ettok gateway unit(s) found:")
    for name, path, is_system in legacy:
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: ettok gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo ettok gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    ettok gateway uninstall")
    print_info("    sudo ettok gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: ettok gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    """Install and start a user-scope gateway service without prompting (``ettok setup``/``import``).
    ...
    if is_container():
        ...
        print_info("   ettok gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: ettok gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: ettok gateway install")
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Ettok install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo ettok gateway uninstall --system")
    print_info("         ettok gateway install")
    print_info("         ettok gateway start")
...
def systemd_install(
    ...
    print()
    print("Next steps:")
    print(f"  {sudo}ettok gateway start{scope_flag}              # Start the service")
    print(f"  {sudo}ettok gateway status{scope_flag}             # Check status")
...
def _print_service_not_installed(system: bool) -> None:
    ...
    print(f"  Run: {sudo}ettok gateway install{scope_flag}")
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `ettok gateway status` or logs for final shutdown state."
        )
        return
...
def _systemd_reset_and_run(action: str, *, system: bool, previous_pid) -> None:
    ...
    try:
        ...
        print(
            ...
            "check `ettok gateway status` or logs for final state."
        )
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {sudo}ettok gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if result.stdout.strip() == "active":
        ...
        print(f"  Run: {sudo}ettok gateway start{scope_flag}")
    ...
    if active_state == "activating" and unit_props.get("SubState", "") == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {sudo}ettok gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {user_flag}reset-failed {svc} && {sudo}ettok gateway start{scope_flag}")
...
# at all and we degrade to a detached background process (the `nohup ettok gateway run` workaround). See
# #23387.
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    (launchd ProgramArguments only) adds ``--external-supervisor`` so ``ettok update`` hands back to
    ...
    ``--external-supervisor`` so ``ettok update`` sees the flag on the live grandchild argv and hands the
...
def _spawn_detached_gateway() -> bool:
    ...
    the `nohup ettok gateway run --replace` workaround but keeps it CLI-managed: stdout goes to
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(f"  Try manually: nohup ettok gateway run --replace > {_dhh()}/logs/gateway.log 2>&1 &")
    if exit_on_failure:
        sys.exit(1)
    return False
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Ettok install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  ettok gateway status             # Check status")
    from hermes_constants import display_hermes_home as _dhh
...
def launchd_stop():
    ...
    _mark_planned_stop()
    # bootout unloads the definition so KeepAlive doesn't respawn; `ettok gateway start` re-bootstraps.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # only stalls the restart (and `ettok update`). Bounded SIGTERM → SIGKILL, ~10s.
...
def launchd_status(deep: bool = False):
    ...
    # Ettok PID may be a detached fallback process; when launchd IS supervising both PIDs match — don't double-count.
    from gateway.status import get_running_pid
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Ettok install")
    else:
        print("⚠ Service definition is stale relative to the current Ettok install")
        print("  Run: ettok gateway start")
    ...
    if not service_listed:
        ...
        if fallback_pid:
            ...
            print("  Cron jobs will fire. Stop with: ettok gateway stop")
        else:
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    print("  Use 'ettok gateway restart' to replace it,")
    print("  or 'ettok gateway stop' first.")
    print("  Or use 'ettok gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error("Refusing to run the Ettok gateway as root inside the official Docker image.")
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # exit-code-75 respawn (stale-code or /restart), which bypasses `ettok gateway restart`.
    if supports_systemd_services():
        try:
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ℹ Ettok Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    {
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL",
             ...
             "password": False, "help": "Channel ID where Ettok delivers cron results and notifications."},
    ...
    {"key": "weixin", "label": "Weixin / WeChat", "emoji": "💬", "token_var": "WEIXIN_ACCOUNT_ID"},
    {
        ...
        "setup_instructions": [
            ...
            "5. Ettok connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: ettok pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Ettok will connect automatically over WebSocket",
        ],
        "vars": [
...
_UNAUTHORIZED_ACCESS_CHOICES = {
    ...
    False: (1,
        ...
        "Use DM pairing (unknown users request access, you approve with 'ettok pairing approve')",
...
def _prompt_unauthorized_access(*, is_email: bool) -> None:
    ...
    if access_idx == 0:
        ...
        print_info("  Approve with: ettok pairing approve <platform> <code>")
    elif is_email:
        print_success("  Unknown email senders will be ignored.")
    else:
        print_info("  Skipped — configure later with 'ettok gateway setup'")
...
def _setup_weixin():
    ...
    print()
    _print_info_lines(
        "  1. Ettok will open Tencent iLink QR login in this terminal.",
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `ettok gateway setup`.")
        return
    ...
    if access_idx == 2:
        ...
        if access_idx == 0:
            print_info(
                "  Unknown DM users can request access and you approve them with `ettok pairing approve`."
            )
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown users can request access; approve with `ettok pairing approve`.")
    elif access_idx == 1:
...
def _setup_service_action(
    ...
    try:
        ...
        if backend is not None:
            ...
            print_info("Start manually: ettok gateway")
    except UserSystemdUnavailableError as e:
...
_WIZARD_NO_SERVICE_LINES = {
    "wsl": (
        "  WSL detected but systemd is not running.", "  Run in foreground: ettok gateway run",
        "  For persistence:   tmux new -s hermes 'ettok gateway run'",
        "  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'",
    ),
    "termux": (
        "  Termux does not use systemd/launchd services.", "  Run in foreground: ettok gateway run",
        "  Or start it manually in the background (best effort): nohup ettok gateway run >{home}/logs/gateway.log 2>&1 &",
    ),
    "unsupported": (
        "  Service install not supported on this platform.", "  Run in foreground: ettok gateway run",
    ),
}
...
def _wizard_install_service(backend: str) -> None:
    ...
    if not (start_now or start_on_login):
        ...
        print_info("  You can install later: ettok gateway install")
        if supports_systemd_services():
            print_info("  Or as a boot-time service: sudo ettok gateway install --system")
        print_info("  Or run in foreground:  ettok gateway run")
        return
    try:
        ...
        print_info("  You can try manually: ettok gateway install")
...
def gateway_setup():
    ...
    if any(_is_progress(_platform_status(p)) for p in _all_platforms()):
        ...
        print()
        print_info("No platforms configured. Run 'ettok gateway setup' when ready.")
...
def _refuse_from_inside_gateway(verb: str, reason: str) -> None:
    ...
    if _is_supervised_gateway_process():
        print_error(
            ...
            f"Use `ettok gateway {verb}` from a shell outside the running gateway."
        )
        sys.exit(1)
...
_WSL_FOREGROUND_HINT = (
    "", "  ettok gateway run                              # direct foreground",
    "  tmux new -s hermes 'ettok gateway run'         # persistent via tmux",
    "  nohup ettok gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # background",
)
...
_NO_BACKEND_MESSAGES = {
    ("install", "termux"): (1,
        "Gateway service installation is not supported on Termux.", "Run manually: ettok gateway"),
    ("install", "wsl"): (1,
    ...
    ("install", "s6"): (None,
        ...
        "  ettok profile create <name>     # creates the s6 service slot",
        "  hermes -p <name> gateway start   # bring it up via s6",
        "  ettok status                    # see currently-supervised gateways"),
    ("install", "container"): (0,
        ...
        "To run the gateway: ettok gateway run"),
    ("install", "unsupported"): (1,
        "Service installation not supported on this platform.", "Run manually: ettok gateway run"),
    ("uninstall", "termux"): (1,
        "Gateway service uninstall is not supported on Termux because there is no managed service to remove.",
        "Stop manual runs with: ettok gateway stop"),
    ("uninstall", "s6"): (None,
        "Per-profile gateways are auto-unregistered when you delete the profile.", "",
        "  ettok profile delete <name>     # tears down the s6 service slot",
    ...
    ("start", "container"): (0,
        ...
        "Or run the gateway directly: ettok gateway run"),
...
def _install_systemd_from_cli(args, *, force: bool, system: bool, run_as_user) -> None:
    if is_wsl():
        ...
        _print_info_lines(
            "  Consider running in foreground instead: ettok gateway run",
            "  Or use tmux/screen for persistence: tmux new -s hermes 'ettok gateway run'",
        )
        print()
...
def _cmd_restart(args):
    ...
    # detached run below is the restart; bailing here left `ettok gateway restart` a
    ...
    if kind == "systemd" and supports_systemd_services():
        ...
        if linger_ok is not True:
            ...
            _print_lines(
                ...
                "  Then restart the gateway:", "    ettok gateway restart",
            )
            return
    ...
    if service_configured:
        _print_lines(
            ...
            "  Fix the service, then retry: ettok gateway start",
        )
        sys.exit(1)
...
# ``ettok gateway status`` hints for a manually-run / stopped gateway, keyed by host kind.
_STATUS_RUNNING_HINTS = {
    ...
    "windows": ("To install as a Windows Scheduled Task (auto-start on login):", "  ettok gateway install"),
    "other": (
        "To install as a service:", "  ettok gateway install", "  sudo ettok gateway install --system",
    ),
}
_STATUS_STOPPED_HINTS = {
    "termux": (
        "  nohup ettok gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # Best-effort background start",
    ),
    "wsl": (
        "  tmux new -s hermes 'ettok gateway run'         # persistent via tmux",
        "  nohup ettok gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # background",
    ),
    "windows": ("  ettok gateway install  # Install as Windows Scheduled Task (auto-start on login)",),
    "other": (
        "  ettok gateway install  # Install as user service",
        "  sudo ettok gateway install --system  # Install as boot-time system service",
    ),
}
...
def _cmd_status(args):
    ...
    if not snapshot.running and named_profile_served_by_running_multiplexer():
        ...
        print("  Manage it from the default profile: ettok gateway status")
        ...
        if pids:
            ...
            print()
            print("To start:")
            print("  ettok gateway run      # Run in foreground")
...
def _cmd_migrate_legacy(args):
    """Stop, disable, and remove legacy Ettok gateway unit files (e.g. hermes.service)."""
