...
TIPS = [
    ...
    "Ctrl+Z suspends Ettok to the background — run fg in your shell to resume.",
    ...
    "ettok chat -t web,terminal enables only specific toolsets for a focused session.",
    "ettok chat -s github-pr-workflow preloads a skill at launch.",
    "ettok chat -q \"query\" runs a single non-interactive query and exits.",
    "ettok chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "ettok chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hermes --yolo bypasses all dangerous command approval prompts for the entire session.",
    "ettok chat --source telegram tags the session for filtering in ettok sessions list.",
    "hermes -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "ettok doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "ettok dump outputs a compact setup summary — great for bug reports.",
    "ettok config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "ettok config edit opens config.yaml in your default editor.",
    "ettok config check scans for missing or stale configuration options.",
    "ettok sessions browse opens an interactive session picker with search.",
    "ettok sessions stats shows session counts by platform and database size.",
    "ettok sessions prune --older-than 30 cleans up old sessions.",
    "ettok skills search react --source skills-sh searches the skills.sh public directory.",
    "ettok skills check scans installed hub skills for upstream updates.",
    "ettok skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "ettok skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "ettok mcp add github --command npx adds MCP servers from the command line.",
    "ettok mcp serve runs Ettok itself as an MCP server for other agents.",
    "ettok auth add lets you add multiple API keys for credential pool rotation.",
    "ettok completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "ettok logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "ettok backup creates a zip backup of your entire Ettok home directory.",
    "ettok profile create coder creates an isolated profile that becomes its own command.",
    "ettok profile create work --clone copies your current config and keys to a new profile.",
    "ettok update syncs new bundled skills to ALL profiles automatically.",
    "ettok gateway install sets up Ettok as a system service (systemd/launchd).",
    "ettok memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "ettok webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: ettok tools disables unused tools, ettok skills config trims skills down.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Ettok tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'ettok profile create coder' creates the 'coder' command.",
    "ettok profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Ettok prints a resume command with session ID and stats.",
    "ettok sessions export backup.jsonl exports all sessions for backup or analysis.",
    ...
    "ettok skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: ettok cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Ettok runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "ettok gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Ettok via a local macOS server.",
    ...
    # --- MCP ---
    "ettok mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "ettok mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "ettok mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — ettok tools can toggle them per platform.",
    ...
    "ettok chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "ettok plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "ettok acp runs Ettok as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "ettok auth add nous or ettok auth add openai-codex sets up OAuth-based providers.",
    ...
    "ettok status --deep runs deeper diagnostic checks across all components.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Ettok your own.",
    ...
    "Before a gateway session reset, Ettok auto-flushes important facts to memory in the background.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'ettok profile create ls' would be rejected.",
    "ettok profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "ettok claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via ettok tools.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Ettok auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Ettok config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Ettok is working.',
    ...
    # --- Curator ---
    'ettok curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "ettok curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'ettok curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'ettok auth reset <provider> [target] clears cooldowns and exhaustion flags on a whole credential pool, or on one credential by index, id, or label.',
    ...
    'ettok dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'ettok dashboard embeds the full Ettok TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'ettok chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'ettok chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'ettok chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "ettok chat --source tool tags programmatic chats so they don't clutter ettok sessions list.",
    'ettok dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'ettok sessions rename <ID> "new title" renames any past session; ettok sessions delete <ID> removes one.',
    'ettok import restores a full Ettok backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'ettok fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'ettok pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'ettok setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'ettok status --deep runs the full health sweep across every component; plain ettok status is the quick view.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Ettok block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from ettok sessions list by default — set --source explicitly to see them.',
...
# Task-oriented example prompts for the empty composer. Kept generic — Ettok is
