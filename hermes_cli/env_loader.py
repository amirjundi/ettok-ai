"""Helpers for loading Ettok .env files consistently across entrypoints."""
...
# Behavioral routing keys a parent Ettok process injects into child env that silently redirect a profile
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `ettok setup` (or edit the "
...
def load_hermes_dotenv(
    *,
    ...
    # Windows (#73381, #86735). 2. A fresh ``ettok update`` retry just completed a deferred dependency
    ...
    # `ettok setup` before the user switched terminal.backend in config.yaml) silently wins again on every
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    tokens), BEFORE Ettok reads credentials; failures never block startup. Precedence/conflicts/provenance
