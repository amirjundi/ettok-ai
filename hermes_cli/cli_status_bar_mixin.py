...
class CLIStatusBarMixin:
    ...
    def _status_bar_segments(
        ...
        if _ok("model"):
            if styled:
                segs.append([(_SB, " ℹ "), (_STRONG, model_short)])
            else:
                segs.append([("", f"ℹ {model_short}")])
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            parts = ["".join(t for _, t in seg) for seg in segs] or [f"ℹ {model_short}"]
            ...
            return f"ℹ {self.model if getattr(self, 'model', None) else 'Ettok'}"
    ...
    def _get_status_bar_fragments(self):
        ...
        try:
            ...
            frags: list = []
            for seg in segs or [[(_SB, " ℹ "), (_STRONG, snapshot["model_short"])]]:
                if frags:
            ...
            # Battery is the first element when enabled: prepend ahead of the ℹ marker.
            battery_label = snapshot.get("battery_label") or ""
