"""Unified provider-credential lifecycle across every store Ettok reads.
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `ettok auth remove` does: a lingering shell export (or
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    kept 401'ing until the user ran ``ettok auth add <provider> --type api-key`` separately. This makes the
    Desktop save's effect on disk match what ``ettok auth add`` does.
    """
