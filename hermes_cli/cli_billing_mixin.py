...
_CHARGE_ERROR_COPY = {
    ...
    "cli_billing_disabled": "  Remote spending is off for this account — a billing admin can turn it on from the portal's Ettok AI page.",
...
_KILLSWITCH_REASON_OVERVIEW = "A billing admin can turn it on from the portal's Ettok AI page to add funds here."
_KILLSWITCH_REASON_BUY = "A billing admin can turn it on from the portal's Ettok AI page before adding funds."
...
class CLIBillingMixin:
    ...
    def _print_logged_out(self, state, load_failed, cmd) -> None:
        ...
        if state.error:
            ...
            print(f"  Run `ettok portal` to log in, then {cmd}.")
    ...
    def _show_subscription(self):
        ...
        if state.context == "team":  # no personal plan — teams run on a shared balance
            self._block_header("ℹ", "Team subscription")
    ...
    def _subscription_free_catalog(self, state, manage_url):
        ...
        if not tiers:
            ...
            return
        self._block_header("ℹ", "Choose a plan")
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        granted = self._step_up_remote_spending(
            ...
            noninteractive_msg="  Run `ettok portal` and allow Remote Spending, then re-run /subscription.",
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print(f"  🔴 {who} Reconnect to restore — run `ettok portal` to re-authorize.")
        elif isinstance(exc, BillingSessionRevoked) or code == "session_revoked":
            print("  🔴 Your session was logged out. Run `ettok portal` to log in again.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        granted = self._step_up_remote_spending(
            ...
            noninteractive_msg="  Run `ettok portal` and allow Remote Spending, then retry.",
        ...
        if not (fresh.logged_in and fresh.cli_billing_enabled):
            print("  Remote Spending is allowed for this terminal, but it's still off for this org. A billing admin can turn it on from the portal's Ettok AI page, then run /topup again.")
