"""Git plumbing for ``ettok update``: fork/upstream sync, trampoline-git detection, lockfile/EOL churn cleanup, orphan rescue refs, parked-branch assessment, fetch-failure classification.
...
def _print_parked_branch_skip_warning(git_cmd: list[str], cwd: Path, current_branch: str, target_branch: str, reason: str) -> None:
    ...
    print(
        ...
        f"    git -C {cwd} status\n"
        f"    git -C {cwd} checkout {target_branch} && ettok update\n"
...
def _offer_upstream_remote(git_cmd: list[str], cwd: Path, *, assume_yes: bool, input_fn) -> bool:
    ...
    print(
        "\nℹ Your fork is not tracking the official Ettok repository.\n"
...
def _portable_git_candidates() -> list:
    ...
    The Ettok-managed PortableGit tree lives under the SHARED root (``<root>/git/...``), not the
    profile-scoped HERMES_HOME (``<root>/profiles/<name>``), so a profile-scoped ``ettok update`` must look
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    on update; install.ps1 pins ``false`` but older checkouts never got it and only ``ettok update`` can
    ...
    reuses its build-pinned ``install.ps1`` forever — so ``ettok update``, which ships with the checkout
