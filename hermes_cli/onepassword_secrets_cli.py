"""CLI handlers for ``ettok secrets onepassword ...``.

Unlike Bitwarden, the ``op`` binary is NOT auto-installed: 1Password publishes the CLI through OS
package managers and signed installers, so Ettok expects an already-installed, already-
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        Panel.fit(
            ...
            "Ettok resolves [cyan]op://vault/item/field[/cyan] references through your\n"
    ...
    console.print(
        "  Map credentials:  [cyan]ettok secrets onepassword set OPENAI_API_KEY "
        "\"op://Private/OpenAI/api key\"[/cyan]\n"
        "  Preview:          [cyan]ettok secrets onepassword sync[/cyan]\n"
        "  Status:           [cyan]ettok secrets onepassword status[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]ettok secrets onepassword setup[/cyan] to enable.")
        return 0
    if binary and not token_set:
        ...
        if who:
            ...
            console.print(
                ...
                "Ettok will warn and skip 1Password on next startup.[/yellow]"
            )
    if not references:
        console.print(
            "\n  [yellow]No references mapped yet.[/yellow]  Add one: "
            "[cyan]ettok secrets onepassword set ENV_VAR \"op://…\"[/cyan]"
        )
    return 0
...
def cmd_set(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "[cyan]ettok secrets onepassword setup[/cyan] to turn it on.[/yellow]"
        )
    return 0
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    return rotate_token(
        ...
        disabled_note=None if op_cfg.get("enabled") else (
            ...
            "run `ettok secrets onepassword setup` to turn it on.[/yellow]"
        ),
    )
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not references:
        console.print(
            ...
            "`ettok secrets onepassword set ENV_VAR \"op://…\"`.[/yellow]"
        )
        return 0
...
def cmd_disable(args: argparse.Namespace) -> int:
    return disable_secret_source(
        ...
        "next Ettok invocation.\n"
        "  Your reference mappings are left in config.yaml — remove them with "
        "[cyan]ettok secrets onepassword remove ENV_VAR[/cyan] if you no longer "
        "need them.",
    )
