...
# Ettok session ids are timestamps (20260812_135332_ab12cd): the strongest sentinel for schema-less rows.
...
# caught exactly this shell converting a live Ettok state.db into two
# generations. A salvage shell must therefore be version-gated, not just
# capability-gated, before it is pointed at (a copy of) a Ettok database.
#
...
_WAL_RESET_VULNERABLE_GUIDANCE = (
    "salvage against a Ettok database with the WAL-reset bug "
...
def _type_conflicts(value: Any, declared: str) -> bool:
    ...
    non-numeric string in an INTEGER column, but Ettok never writes one, so
