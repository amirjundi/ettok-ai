...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    Ettok's plumbing calls. ``core.sshCommand`` is pinned to ``ssh -o BatchMode=yes`` so the ssh
    ...
    Ettok shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def _process_command_is_hermes(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Ettok code."""
    try:
        import psutil
...
def pid_is_hermes(pid: int, *, expected_start_time: int | None = None) -> bool:
    ...
    The PID must be valid, currently exist, and identify a Ettok process. When the caller captured
...
def bounded_probe_run(
    ...
    ``Get-CimInstance Win32_Process`` gateway scan hit exactly this during ``ettok update`` on slow-WMI
