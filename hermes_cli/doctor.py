"""``ettok doctor`` — diagnose (and with --fix, repair) a Ettok install.
...
def _check_auth_providers(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("Auth provider status", "(could not check: {e})"):
        ...
        # Native OAuth is Ettok' own device-code flow; the Codex CLI only imports existing ~/.codex/auth.json
...
def _ack_advisory(ack_target: str) -> None:
    """`ettok doctor --ack <id>`: persist the ack and return without running diagnostics."""
...
def _print_summary(should_fix: bool, total: Finding) -> None:
    ...
    if should_fix and total.fixed > 0:
        ...
        print(numbered)
        if not should_fix:
            print(color("  Tip: run 'ettok doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
