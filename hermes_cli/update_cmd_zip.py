"""ZIP-download fallback for ``ettok update`` (Windows with broken git): two-phase stage/commit swap, dirty-tree guard.
...
_STASH_HINT = "  Stash or commit your changes, then rerun `ettok update`."
...
def _download_and_swap_zip(branch: str, zip_url: str) -> None:
    ...
    try:
        ...
        print("  Re-run `ettok update` to retry; if the agent won't start, reinstall from https://hermes-agent.nousresearch.com")
        _m().sys.exit(1)
    finally:
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `ettok update "
            f"--branch {branch}`, or update against main with `ettok update`."
        )
        _m().sys.exit(1)
    ...
    if not import_ok:
        ...
        print("  Re-run `ettok update` to complete it.")
        _m().sys.exit(1)
