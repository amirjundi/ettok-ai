...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: ettok gateway install\n"
                "                    sudo ettok gateway install --system  # Linux servers\n"
                "     Check status:  ettok cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        print(color("No scheduled jobs.\nCreate one with 'ettok cron create ...' "
...
def cron_tick():
    ...
    try:
        ...
        print("  Check `ettok cron status` and the gateway log for details.")
        return 1
    return 0
...
def cron_incidents(args) -> int:
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(color("✗ Incident ID required: ettok cron incidents ack <incident_id>", Colors.RED))
            return 1
    ...
    for inc in incidents:
        ...
        print()
    print(color(f"  {len(incidents)} incident(s)  |  ack one with: ettok cron incidents ack <id>",
                Colors.DIM))
    return 0


_PERMISSION_HINT = ("  Hint: jobs.json may be owned by another user (e.g. rewritten by a root "
                    "`docker exec hermes ettok cron ...`). Fix ownership to match the gateway "
...
def _print_ticker_health(pids: list) -> None:
    ...
    if hb_age is None:
        ...
        print("  Cron jobs will NOT fire until the ticker writes its first heartbeat.\n"
              "  If the gateway just started, wait ~60s and re-run `ettok cron status`.\n"
              "  If heartbeat never appears, restart: ettok gateway restart")
        ...
        print("  Cron jobs may NOT be firing. Restart: ettok gateway restart")
    ...
    if provider != "builtin":
        ...
        if pids or gateway_alive_via_lock:
            ...
            print("\n  To enable automatic execution:\n"
                  "    ettok gateway install    # Install as a user service\n"
                  "    sudo ettok gateway install --system  "
                  "# Linux servers: boot-time system service\n"
                  "    ettok gateway            # Or run in foreground")
...
def cron_doctor() -> int:
    ...
    for job, issues in findings:
        ...
        for issue in issues:
            print(f"    - {issue}")
    print()
    print(color("Next: fix the listed job config, then run `ettok cron doctor` again.", Colors.DIM))
    return 1
...
def cron_notepad(args) -> int:
    """Handle ``ettok cron notepad <job_id> [get|set|delete|list]`` (per-job durable KV).
    ...
    try:
        ...
        if key is None or (action == "set" and value is None):
            print(color(f"Usage: ettok cron notepad <job_id> {usage_args}", Colors.RED))
            return 1
        if action == "set":
...
def cron_command(args):
    ...
    print(f"Unknown cron command: {subcmd}\n"
          "Usage: ettok cron [list|create|edit|pause|resume|run|remove|status|runs|doctor|tick]")
    sys.exit(1)
