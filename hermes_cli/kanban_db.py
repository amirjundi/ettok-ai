...
Lives under the shared Ettok root: ``default`` board DB at ``<root>/kanban.db`` (pre-boards
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    """Per-board worker log dir (logs follow the board so ``ettok kanban log``
...
# Canonical sort-order mappings for ``ettok kanban list --sort``.
...
# dispatcher tick or a manual `ettok kanban recompute` (issue #22459).
...
def promote_task(
    ...
    if unsatisfied:
        return False, (
            ...
            f"`ettok kanban unlink <parent_id> {task_id}`)"
        )
