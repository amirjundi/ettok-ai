...
def build_acp_parser(subparsers, *, cmd_acp: Callable) -> None:
    ...
    acp_parser = subparsers.add_parser(
        "acp", help="Run Ettok AI as an ACP (Agent Client Protocol) server",
        description="Start Ettok AI in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    add_accept_hooks_flag(acp_parser)
    acp_parser.add_argument(
        "--version", action="store_true", dest="acp_version",
        help="Print Ettok ACP version and exit")
    acp_parser.add_argument(
        "--check", action="store_true",
        help="Verify ACP dependencies and adapter imports, then exit")
    acp_parser.add_argument(
        "--setup", action="store_true",
        help="Run interactive Ettok provider/model setup for ACP terminal auth")
    acp_parser.add_argument(
