"""``ettok egress`` subcommand parser."""
...
def build_egress_parser(subparsers) -> None:
    ...
    # OUTBOUND egress firewall (iron-proxy); `ettok proxy` is the separate INBOUND
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        # dest='egress_command' stays disjoint from ``ettok proxy`` (dest='proxy_command').
