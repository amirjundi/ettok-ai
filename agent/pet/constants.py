...
inferred from the atlas shape so Ettok renders both legacy 8-row sheets and
...
class PetState(str, Enum):
    """Animation state a pet can be shown in (Ettok names; Codex rows say ``jumping``/``running`` for ``jump``/``run``)."""
...
# Legacy Ettok/petdex row order (top -> bottom) for the older 8-row, 9-column atlas.
...
# Canonical Ettok names -> accepted row-name aliases in descending preference.
