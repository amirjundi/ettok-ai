"""Unified removal contract for every credential source Ettok reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``ettok auth remove <provider> <N>`` must make the entry stay gone across
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Ettok.",
            f"  The pool entry is now suppressed — Ettok will ignore "
            f"{env_var} until you run `ettok auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    ...
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Ettok Anthropic OAuth credentials")
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `ettok model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `ettok auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `ettok auth add copilot` to re-enable if needed.",
    ])
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `ettok auth add anthropic` to re-enable if needed.",
        ),
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `ettok auth add qwen-oauth` to re-enable if needed.",
        ),
