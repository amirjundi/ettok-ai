"""Ettok-tools-as-MCP server for the codex_app_server runtime.

Codex owns the loop and tool list there, so a curated subset of Ettok tools is
...
# Each name MUST match a registered Ettok tool ``model_tools.handle_function_call()`` can dispatch.
...
def _build_server() -> Any:
    """Create the MCP server with Ettok tools attached (lazy imports: importable without ``mcp``)."""
    try:
    ...
    mcp = MCPServer(
        "hermes-tools",
        instructions=(
            "Ettok AI's tool surface, exposed for use inside a Codex "
    ...
    # Authoritative Ettok schemas so MCP clients see the same parameter docs the model does.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug("skipping %s — not registered in this Ettok process", name)
            continue
        description = spec.get("description") or f"Ettok {name} tool"
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # Keep Ettok' own banners off stdout (the MCP wire).
