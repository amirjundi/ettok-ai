...
def _reasoning_config_for_model(model: str, reasoning_config: dict | None) -> dict | None:
    """Clamp Ettok' extended effort set (``ultra``) to the OpenAI-compat wire vocabulary.

    Ettok' internal effort set extends the wire vocabulary with ``ultra`` (the /reasoning command documents
...
def _build_gemini_thinking_config(model: str, reasoning_config: dict | None) -> dict | None:
    """Translate Ettok/OpenRouter-style reasoning config to Gemini thinkingConfig."""
    if not isinstance(reasoning_config, dict):
        return None
    ...
    # is stricter (low/high). Clamp Ettok' wider effort set to what each
