"""OpenAI-shape bridge shared by Ettok' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Ettok' tool schemas travel INTO the
...
read/edit/execute tools forwards only Ettok' agent-level tools, since re-offering overlapping ones
makes Ettok redo finished work.
"""
...
class StreamChunks(list):
    ...
    Ettok reads off the ``create`` result; a plain list would drop them on the stream path."""
