...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are Ettok AI, built by Nous Research. Be direct: match the length of your reply to the weight of the ask "
...
HERMES_AGENT_HELP_GUIDANCE = (
    ...
    "You are Ettok AI. You run on the Ettok AI runtime (by Nous Research), which Ettok AI is built on. "
...
KANBAN_GUIDANCE = (
    ...
    "sits in `ready` forever). Ground every assignee in a real profile (`ettok profile list`, or ask the user), and "
    "express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n\n"
    "- Do not shell out to `ettok kanban <verb>` for board operations. Use the `kanban_*` tools — they work across "
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Ettok appends their message to the end of a tool result, wrapped exactly as:\n"
    f"{STEER_MARKER_OPEN}\n<their message>\n{STEER_MARKER_CLOSE}\n"
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    gated = (
        (True,
         "[Note: this message came from HUD mode — a small floating Ettok "
         ...
         "rather than anything inside Ettok. read_window_below identifies that app."),
        (True,
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Ettok terminal UI (TUI). Files: there is no attachment channel and MEDIA:/path tags "
...
def _remote_backend_hint(backend: str) -> str:
    ...
    if probe:
        return lead + (
            f"this {backend} environment — NOT on the machine where Ettok itself is running. The host OS, "
            f"home, and cwd of the Ettok process are irrelevant; only the following backend state matters:\n{probe}"
        )
    description = (
    ...
    return lead + (
        f"{description} — NOT on the machine where Ettok itself runs. The backend probe didn't respond at "
...
RUNTIME_ENVIRONMENT_HEADING = "# Ettok runtime environment"
RUNTIME_ENVIRONMENT_END = "<!-- End Ettok runtime environment -->"
...
def build_context_files_prompt(
    ...
    # An explicitly configured cwd is honored verbatim — the Ettok tree is a legitimate workspace when the
    # user deliberately points a session at it — and CLI-style surfaces pass
    # allow_install_tree_fallback=True because their launch dir IS the user's shell cwd (developing Ettok
    # in-tree). See #64590.
    from agent.runtime_cwd import _is_install_tree
    if cwd is None and not allow_install_tree_fallback and _is_install_tree(cwd_path):
        logger.warning(
            "skipping project-context discovery: working-directory resolution fell back to the Ettok "
