...
def _workspace_has_runnable_recipe(root: Any) -> bool:
    """Whether ``ettok verify`` has a runtime recipe here: a saved
    ...
    if verify_commands:
        ...
        if has_recipe:
            command_instruction += (
                ...
                "start + readiness), prefer `ettok verify --json` — a passing "
                "run records verification evidence for this workspace."
            )
    elif has_recipe:
        command_instruction = (
            "No canonical test/lint/build command was detected, but the "
            "project has a runnable verification recipe. Run `ettok verify "
