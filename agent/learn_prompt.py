...
``skill_manage`` per the Ettok authoring standards; large prose sources get the knowledge-base layout (lean SKILL.md
...
Follow the Ettok skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Ettok`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Ettok.
...
4. "## How to Run" — the canonical invocation, framed through Ettok tools.
...
Ettok-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference Ettok tools by name in backticks: `terminal`, `read_file`,
