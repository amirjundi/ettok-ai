...
def _codex_native_auto_compaction(agent: Any) -> bool:
    """Codex app-server threads are compacted by the codex agent itself; Ettok only
...
def _preflight_compression(
    ...
    if _preflight_deferred:
        ...
        logger.info(
            "Skipping Ettok preflight compression for codex app-server "
            "(mode=%s); Ettok will not start thread compaction here.",
