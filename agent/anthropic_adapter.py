"""Anthropic Messages API adapter: client construction + the Messages call for Ettok's
...
# Ettok effort -> Anthropic adaptive-thinking effort (output_config.effort). 4.7+ exposes
...
# Anthropic's OAuth billing classifier fingerprints certain Ettok tool schemas/prose as a
...
def _attribution_headers() -> Dict[str, str]:
    ...
    return {
        "HTTP-Referer": "https://hermes-agent.nousresearch.com", "X-Title": "Ettok AI",
...
def _normalize_to_mcp_wire(name: str) -> str:
    ...
    Ettok tools (``read_file``) and native MCP tools registered as ``mcp_<server>_<tool>`` must
...
_OAUTH_SYSTEM_REPLACEMENTS = (
    ("Ettok AI", "Claude Code"), ("Ettok agent", "Claude Code"),
...
def _thinking_kwargs(reasoning_config: Dict[str, Any], model: str, effective_max_tokens: int) -> Dict[str, Any]:
    ...
    4.7+ ``thinking.display`` defaults to "omitted", hiding the reasoning Ettok shows in its CLI,
...
def build_anthropic_kwargs(
    ...
    # which silently hides reasoning text that Ettok surfaces in its CLI. We request "summarized" so the
