...
# along or a child that imports Ettok code loses the Kanban mutation guard
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    # mode changes CWD. Ettok's root is added ONLY when the child runs in Ettok's Python env —
    # exposing Ettok's site-packages to an external interpreter can mix incompatible compiled
    # extensions (3.12 NumPy under a 3.9 venv). Inherited Ettok-owned entries are stripped first.
    # Before re-injecting PYTHONPATH, strip Ettok-owned entries that leaked through _scrub_child_env
    # (PYTHONPATH is in _SAFE_ENV_PREFIXES so it passes the scrub). They are redundant for same-Ettok-
    ...
    if _uses_hermes_python_environment(child_python):
        ...
        logger.info("execute_code: child interpreter %s is outside the Ettok "
...
# Interpreter paths already reported as outside the Ettok environment.
...
def _uses_hermes_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Ettok's active Python environment. Short-circuits when
