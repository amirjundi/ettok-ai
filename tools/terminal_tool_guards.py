...
_FOREGROUND_GUIDANCE = (
    (
        ...
        "notify_on_complete=true) so Ettok tracks the process, then run readiness "
...
def gateway_lifecycle_block(
    *,
    ...
    ``systemctl``/``launchctl``/``ettok gateway restart|stop|uninstall``
    ...
    an unsupervised foreground ``ettok gateway run`` has no KeepAlive to turn
    ...
    if lifecycle_scan_root_within_budget(command) and contains_launchctl_submit_command(command):
        return _blocked_json(
            ...
            "or determine whether it is independent of Ettok. Perform authorized "
    ...
    if contains_gateway_lifecycle_command_or_referenced_script(
        ...
        return _blocked_json(
            ...
            "to child processes). Run `ettok gateway restart` from a "
