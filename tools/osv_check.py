...
# ladders, parked-server self-probes and repeated `ettok mcp test` runs re-run the preflight
...
# The cache is also persisted under the Ettok home so separate processes and gateway
...
def _disk_cache_path() -> Optional[Path]:
    ...
    active profile and is isolated across Ettok homes. The cache directory
    is created on demand. Returns ``None`` when Ettok home cannot be
