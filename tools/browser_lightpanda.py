...
def _http_cache_dir() -> Path:
    """Filesystem HTTP cache shared by every Lightpanda this Ettok spawns.
    ...
    bounds it without Ettok managing eviction.
    """
    ...
    if not binary:
        if os.name == "nt":
            return None, ("browser.engine is 'lightpanda' but Lightpanda has no Windows "
                          "build. Set browser.engine to auto (or run Ettok under WSL2).")
...
def reap_orphaned_lightpanda() -> int:
    """Kill ``lightpanda serve`` processes whose owning Ettok is gone; return the count. A live owner is
