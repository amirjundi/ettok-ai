...
DANGEROUS_PATTERNS = [
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]hermes[^\n]*\.env\b', "access to Ettok secrets (Windows path)"),
    # ── end of Windows tier
    ...
    (r'\bhermes\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart ettok gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "ettok update (restarts gateway, kills running agents)"),
    ...
    # cp/mv/install OVERWRITING a credential/SSH/shell-rc/Ettok file (key implant, login-time
    ...
    # sed -i on Ettok config/.env bypasses the redirection/tee rules; pairs the file_tools
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # later. MUST run before the backslash strip (which would dissolve C:\Users\alice to C:Usersalice). Ettok home
...
_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = "stop/restart ettok gateway via shell-spliced verb (kills running agents)"
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Ettok ad-hoc temp script."""
    try:
