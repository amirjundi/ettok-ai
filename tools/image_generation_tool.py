#!/usr/bin/env python3
"""Image generation via FAL.ai (model picked in ``ettok tools``, persisted to ``image_gen.model``).
...
def _resolve_managed_fal_gateway():
    """Managed gateway config for the stored `ettok tools` selection, or ``None`` for direct FAL.
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `ettok tools` → Image Generation."
...
def _prepare_fal_request(model_id, meta, prompt, aspect_ratio, seed, overrides, source_images):
    ...
    if source_images and not edit_endpoint:
        raise ValueError(
            ...
            f"via `ettok tools` → Image Generation.")
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed:
        lines.append("  2. Sign in to a Nous account that has the managed FAL gateway enabled "
                     "(`ettok setup`)")
    lines.append("  3. Configure a different image_gen provider via `ettok tools` → Image Generation "
                 "(run `ettok plugins list` to see installed backends)")
    return "\n".join(lines)
...
def _dispatch_to_plugin_provider(
    ...
    if provider is None:
        return _provider_error(
            ...
            f"Run `ettok plugins list` to see available image gen backends.", "provider_not_registered")
    ...
    try:
        ...
        if is_type_error and ("image_url" in kwargs or "reference_image_urls" in kwargs):
            ...
            return _provider_error(
                ...
                f"text-to-image, or pick a backend that supports editing via `ettok tools` → "
