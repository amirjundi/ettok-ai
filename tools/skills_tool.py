...
def _org_provenance_header(skill_dir: Path, active_skills_dir: Path):
    ...
    header = (
        ...
        "> `ettok sync propose` (or automatically, if your org enables it).\n\n")
    ...
    if skill_md is not None and project_dirs:
        ...
        if _under_any(skill_md, project_dirs) and is_quarantined_project_skill(skill_md):
            return _fail(
                ...
                "`ettok skills untrust`."), None, None
...
def skill_view(
    ...
    try:
        ...
        if _is_skill_disabled(resolved_name):
            return _fail(f"Skill '{resolved_name}' is disabled. Enable it with `ettok skills` or inspect the files directly on disk.")
