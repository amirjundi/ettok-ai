...
def _embedded_daemon_spawn_command(driver_cmd: str, serve_args: List[str], *, platform: str,
    ...
    if not resolved_app:
        raise RuntimeError("CuaDriver.app is required for private computer-use sessions on macOS. Run `ettok computer-use install` to restore it.")
...
class _EmbeddedCuaDaemon:
    ...
    startup, so reusing the machine-wide daemon would let one Ettok session's YOLO choice affect another. A
    ...
    Ettok YOLO (``--dangerously-bypass-approvals``); ``bounded`` = a user-reviewed capability manifest approved
