...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a Ettok-managed provider credential per
    ...
    in the ``execute_code`` child); non-Ettok keys (TENOR_API_KEY, …) stay
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    skill's ``required_environment_variables``). Ettok-managed provider credentials
    ...
    for name in _accepted((n.strip() for n in var_names), (
        "env passthrough: refusing to register Ettok provider "
...
def _accepted(names, refusal_msg: str):
    """Yield non-empty *names* that are not Ettok provider credentials; refused
...
def _load_config_passthrough() -> frozenset[str]:
    ...
    try:
        ...
        result.update(_accepted((i.strip() for i in items if isinstance(i, str)), (
            "env passthrough: refusing to register Ettok "
