...
Incident (2026-08-17): a malformed-SCHEMA state.db sent Ettok into an
...
def test_live_connection_keeps_its_write_lock_across_a_repair_pass(tmp_path):
    ...
    POSIX advisory locks, so it is immune. DELETE mode is what Ettok falls
...
def test_forensic_backup_includes_the_rollback_journal(tmp_path):
    ...
    Rollback-journal mode is Ettok's fallback on NFS/SMB/FUSE/ZFS and on
