...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Ettok\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Ettok\venv\Scripts\python.exe"
...
# resume_pending=True) never fires when `ettok gateway stop` kills the
