...
commands) emit box-drawing characters and the ℹ glyph, which raise
...
in ``ettok setup`` because the repair returned early. See the Raspberry Pi
...
def test_fallback_when_reconfigure_unavailable(monkeypatch, tmp_path):
    ...
    sys.stdout.flush()
    fh.close()
    assert "ℹ".encode("utf-8") in real_path.read_bytes()
