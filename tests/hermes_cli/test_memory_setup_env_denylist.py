...
other subprocess-influencing or Ettok-runtime-location name) could
otherwise plant a value into ``.env`` via the interactive memory-setup
wizard. The next Ettok process would load it through the
...
def test_explicit_hermes_home_writes_to_that_env_file(tmp_path):
    ...
    Ettok home; keep that target while still routing through
