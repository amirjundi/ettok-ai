...
def _setup_hermes_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Ettok auth store at the given root."""
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``ettok model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``ettok model``
    ...
    assert "ettok model" not in rendered.lower()
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Ettok that didn't validate) must not be silently honored on the refresh
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `ettok model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `ettok auth list` reflects truth and
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``ettok auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `ettok auth remove xai-oauth 1`.
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Ettok invocation."
    )


def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    """remove -> ``ettok model`` re-login (``_login_xai_oauth``) must clear the
    ...
    only ``auth_add_command`` cleared it — the ``ettok model`` re-login path did
    not. So after remove -> re-login the seed kept skipping and ``ettok auth
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`ettok auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``ettok auth status``, ``ettok setup``, and the
