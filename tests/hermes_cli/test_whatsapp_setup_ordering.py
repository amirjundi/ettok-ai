...
Before the fix, ``ettok whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``ettok gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``ettok whatsapp`` with an existing paired session and
