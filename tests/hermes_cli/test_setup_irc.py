"""Tests for IRC gateway configuration via `ettok setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Ettok install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Ettok install."""
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `ettok setup gateway` experience with IRC present."""
