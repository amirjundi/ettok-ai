"""Tests for ``ettok dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``ettok update``
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        ...
        assert "No ettok dashboard or serve processes running" in out
    ...
    def test_status_with_processes(self, capsys):
        ...
        # couldn't see (#81564).
        processes = [
            (12345, "ettok dashboard --port 9119"),
            (12346, "python -m hermes_cli.main dashboard --host 0.0.0.0 --port 9120"),
            (12347, "ettok serve --host 100.94.65.93 --port 9119"),
        ]
        ...
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "3 ettok dashboard/serve process(es) running" in out
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        # for the post-`ettok update` path).
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``ettok dashboard --stop`` must not end up ALSO starting
    a new server."""
...
class TestArgparseWiring:
    ...
    ``ettok dashboard --stop`` / ``--status`` actually parse."""
