...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    # `ettok auth add nous` must also populate providers.nous so the
    ...
    # raises "Ettok is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`ettok auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``ettok auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``ettok auth list`` showed two labels sharing
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """ettok auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``ettok auth add xai-oauth`` runs must produce independent pool entries.
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`ettok logout --provider openai-codex` must still clear model.provider.
...
# =============================================================================
# Unified credential-source stickiness — every source Ettok reads from has a
