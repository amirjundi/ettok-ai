"""Regression test: ``ettok mcp add --command`` must not clobber the
...
result, ``ettok mcp add foo --url ...`` silently launches chat instead
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`ettok mcp add foo --url ...` must keep args.command == "mcp".
