"""Tests for ``ettok debug`` CLI command and debug utilities."""
...
class TestMissingLogNote:
    ...
    `ettok debug share` runs on the backend, so a desktop connected to a
...
class TestRunDebugShare:
    ...
    def test_share_uploads_five_pastes(self, hermes_home, capsys):
        ...
        with patch("hermes_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- ettok dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
...
class TestScheduleAutoDelete:
    ...
    handle the DELETE requests synchronously on the next ``ettok debug``
    invocation.
    """
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_output_warns_on_dpaste_fallback(self, hermes_home, capsys):
        ...
        or a working `ettok debug delete` (#106164)."""
        from hermes_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``ettok debug share`` (CLI) and ``POST /api/ops/debug-share``
...
class TestShareConsentGate:
    """`ettok debug share` requires explicit consent before uploading.
