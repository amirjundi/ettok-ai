"""Orphan Desktop-local ``ettok serve`` reap at backend start.
...
def test_desktop_local_serve_shape_matches_ephemeral_loopback():
    ...
    assert _is_desktop_local_serve_cmdline(
        "ettok serve --isolated --host 127.0.0.1 --port 0 --ssh-owner-nonce abc"
    )
    assert _is_desktop_local_serve_cmdline(
        "/venv/bin/ettok serve --host=127.0.0.1 --port=0"
    )


def test_desktop_local_serve_shape_spares_fixed_port_and_non_serve():
    assert not _is_desktop_local_serve_cmdline(
        "ettok serve --host 100.106.105.2 --port 9119 --skip-build"
    )
    assert not _is_desktop_local_serve_cmdline(
        "ettok serve --host 127.0.0.1 --port 9119"
    )
    assert not _is_desktop_local_serve_cmdline("ettok gateway run --replace")
    assert not _is_desktop_local_serve_cmdline(
        "vim notes about ettok serve --port 0"
    )


def test_reap_only_kills_ppid1_local_serves():
    scanned = [
        (111, "ettok serve --host 127.0.0.1 --port 0"),  # orphan local
        (222, "ettok serve --host 127.0.0.1 --port 0"),  # still has parent
        (333, "ettok serve --host 100.1.2.3 --port 9119"),  # fixed remote
        (444, "ettok serve --isolated --host 127.0.0.1 --port 0"),  # orphan isolated
    ]
...
def test_reap_spare_lock_owned_ssh_remote_backend_of_foreign_client():
    ...
    scanned = [
        (555, "ettok serve --host 127.0.0.1 --port 0"),  # lock-owned remote
        (666, "ettok serve --host 127.0.0.1 --port 0"),  # genuine orphan
    ]
    ppids = {555: 1, 666: 1}
    terms: list[int] = []
    live = {555, 666}
...
def test_reap_spares_young_backend_until_desktop_can_write_lock():
    ...
    scanned = [(777, "ettok serve --isolated --host 127.0.0.1 --port 0")]
    terms: list[int] = []
...
def test_reap_spares_backend_when_process_age_is_unknown():
    scanned = [(778, "ettok serve --isolated --host 127.0.0.1 --port 0")]
    terms: list[int] = []
...
def test_reap_age_boundary_makes_180_second_orphan_eligible():
    scanned = [
        (779, "ettok serve --isolated --host 127.0.0.1 --port 0"),
        (780, "ettok serve --isolated --host 127.0.0.1 --port 0"),
    ]
    terms: list[int] = []
    live = {779, 780}
...
def test_reap_spare_lock_owned_backend_even_without_exclude_match(tmp_path):
    ...
    scanned = [(4242, "ettok serve --host 127.0.0.1 --port 0")]
    terms: list[int] = []
