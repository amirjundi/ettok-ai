...
the SAME provider universe as ``ettok model`` (the CLI/TUI picker).
...
def test_every_hermes_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `ettok model` universe."""
    ...
    assert not missing, (
        "providers shown in `ettok model` but not configurable in the desktop "
