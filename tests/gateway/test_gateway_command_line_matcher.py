...
Regression guard for the Windows ``ettok gateway restart`` silent-outage bug:
...
ACCEPT = [
    ...
    "hermes-gateway.exe",
    "ettok gateway",          # bare `ettok gateway` defaults to run
    "ettok gateway run",
    # profile selector AFTER the `gateway` token (argv is profile-position
    # agnostic — _apply_profile_override strips --profile/-p anywhere)
    "ettok gateway --profile work run",
    ...
    r'"C:\Program Files\Ettok\gateway\run.py" run',
