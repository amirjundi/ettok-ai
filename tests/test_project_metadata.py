...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `ettok update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `ettok update` downgrades the "
...
def test_every_lazy_deps_exact_pin_matches_uv_lock():
    ...
    install ships the locked version, and the ``ettok update`` lazy-refresh
    ...
    assert not drift, (
        ...
        "every package the core lock also ships — otherwise `ettok update` "
...
def test_huggingface_hub_lazy_pin_matches_uv_lock():
    ...
    a feature from mere package presence, the `ettok update` lazy-refresh
    ...
    assert lazy_pin == locked, (
        ...
        "huggingface-hub`), or `ettok update` will churn/downgrade the "
