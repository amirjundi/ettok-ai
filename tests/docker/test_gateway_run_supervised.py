...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → ettok gateway run) can take
    ...
    # (main-wrapper.sh → python → ettok gateway run) can take well over
...
def test_supervised_gateway_does_not_recurse(
    ...
    supervised ``ettok gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``ettok gateway
    ...
    # (main-wrapper.sh → python → ettok gateway run) can take well
    ...
    # started either — so we'd see the CMD's `ettok gateway run`
    ...
    # `ettok gateway run`. If the recursion guard fails, s6 would
    ...
    assert n <= 1, (
        f"expected at most one supervised python `ettok gateway run` "
