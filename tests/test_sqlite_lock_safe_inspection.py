"""POSIX advisory locks must survive Ettok' own database inspection.
...
Ettok used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `ettok sessions
...
out across Ettok' inspection calls.
"""
