...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `ettok tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``ettok tools`` → Video Generation.",
    ...
    # Full Ettok toolsets (CLI + messaging platforms). All share the core tools;
    ...
    # Mirrors hermes-cli; `ettok tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "hermes-cron": _bundle("Default cron toolset - same core tools as hermes-cli; gated by `ettok tools`"),
