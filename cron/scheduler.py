...
# `ettok update`) otherwise fail with ModuleNotFoundError for hermes_time et al.
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `ettok fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `ettok cron pause {job_ref}` "
...
class CronTickYielded(RuntimeError):
    ...
    open). Raised, not returned, so ``record_ticker_error`` sees it and ``ettok cron status``
    isn't green.
    """
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    # (interrupted `ettok update`, manual git pull) serves MIXED modules — old entries frozen in
    ...
    if provider_reachable and re.search(
        ...
        if skew is not None:
            ...
            message += (
                ...
                "`ettok gateway restart` to fix it."
            )
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    A cron tick can fire while the gateway is tearing down — SIGTERM from ``ettok update`` / ``hermes
...
def _get_hermes_home() -> Path:
    """Ettok home at call time (honouring the test override). Cron is per-profile: never freeze
...
def _snapshot_pin(job: dict, axis: str, current: str, job_id: str) -> str:
    ...
    if snapshot and current and snapshot.lower() != current.lower():
        logger.info(
            ...
            "`ettok cron edit %s --%s <value>` or cron.%s in config.yaml moves it.",
...
def _load_cron_job_config(job: dict, job_id: str, job_name: str) -> _CronJobConfig:
    ...
    ``ettok cron edit --model`` applies next tick."""
    ...
    if not (isinstance(model, str) and model.strip()):
        raise RuntimeError(
            ...
            f"`ettok cron edit {job_id} --model <name>` or set a "
            "default with `ettok model <name>`."
        )
...
def _worktree_maintenance_repos() -> List[str]:
    ...
    # Ettok source checkout (git installs only; wheel installs have no .git).
    with contextlib.suppress(Exception):
...
def _maybe_reap_dead_owners() -> None:
    ...
    # recovery previously ran only at scheduler STARTUP. A one-shot `ettok cron run` that claimed a job and
...
def tick(
    ...
    try:
        # `ettok pause` ESTOP: skip dispatch, never touch in-flight runs; check_paused logs once.
