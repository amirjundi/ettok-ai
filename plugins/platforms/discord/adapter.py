...
class DiscordAdapter(DiscordMediaMixin, BasePlatformAdapter):
    ...
    async def send_update_prompt(
        ...
        def _build(_channel):
            ...
            embed = discord.Embed(
                title="ℹ Update Needs Your Input", description=f"{prompt}{default_hint}", color=discord.Color.gold(),
            )
            view = UpdatePromptView(
                session_key=session_key, allowed_user_ids=self._allowed_user_ids,
                allowed_role_ids=self._allowed_role_ids,
            )
            content = self._self_contained_prompt_content("ℹ **Update Needs Your Input**", f"{prompt}{default_hint}")
            return {"content": content, "embed": embed, "view": view}, view
