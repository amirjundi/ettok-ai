...
# context-local overrides, so the gateway and ``ettok pairing`` CLI wrote different
...
# process (e.g. the ``ettok pairing`` CLI) re-imports this module later with the final environment already
...
def _load_json_file(path: Path) -> dict:
    ...
    try:
        ...
        logger.warning(
            ...
            "If you ran `docker exec <container> ettok pairing approve ...` as root, "
