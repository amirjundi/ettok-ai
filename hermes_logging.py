"""Centralized logging setup for Ettok AI.
...
# in Ettok (TUI, gateway, hy_memory, MCP servers, CLI commands all log) —
...
# Logger name prefixes per component; used by _ComponentFilter and ``ettok logs --component``.
COMPONENT_PREFIXES = {
...
def setup_logging(
    *,
    ...
    """Configure the Ettok logging subsystem; returns the ``logs/`` directory.
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their Ettok home.
